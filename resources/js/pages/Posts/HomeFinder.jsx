...
export default function HomeFinder() {
  const { list, cities, categories, filters: initialFilters } = usePage().props;
  ...
  function handleApplyArea() {
    if (tempAreaRange.min)
      handleFilterChange("minArea", Number(tempAreaRange.min));
    else handleFilterChange("minArea", undefined);

    if (tempAreaRange.max)
      handleFilterChange("maxArea", Number(tempAreaRange.max));
  ...
  function getPriceLabel() {
    if (filters.minPrice && filters.maxPrice) {
      return `${formatPrice(filters.minPrice)} - ${formatPrice(
        filters.maxPrice
      )}`;
        ...
        <div className="list-container">
          ...
          <div className="results-header">
            ...
            <div className="results-info">
              ...
              Có <strong>{list.total?.toLocaleString("vi-VN") ?? 0}</strong>{" "}
          ...
          <p className="page-subtitle">
            Hiện có {list.total?.toLocaleString("vi-VN") ?? 0} bất động sản.
          </p>
          ...
          <div className="results-section" key={list.current_page}>
            {list.data.length > 0 ? (
              <>
                <CardList post={list.data} limit={20} showMore={false} />
              ...
              <div className="options-grid">
                <button
                  ...
                  className={`option-btn ${
                    !filters.minArea && !filters.maxArea ? "active" : ""
                  }`}
                ...
                {areaOptions.map((option, index) => (
                  <button
                    ...
                    key={index}
                    className={`option-btn ${
                      filters.minArea == option.min &&
                      filters.maxArea == option.max
                        ? "active"
                        : ""
                    }`}
