...
export default function Suggest({
    ...
    // ========== Hàm xử lý gợi ý ==========
    function filterSuggestions(titles, keyword) {
        ...
        for (const title of titles) {
            ...
            // ==== 1) ƯU TIÊN: title bắt đầu bằng từ đầu tiên ====
            const startsWithFirstWord = titleWords[0].startsWith(first);

            // ==== 2) HOẶC: chứa từ đầu tiên ở bất kỳ vị trí ====
            const containsFirstWord = normalizedTitle.includes(first);

            if (!startsWithFirstWord && !containsFirstWord) continue;

            // ==== 3) Các từ tiếp theo phải match NGẪU NHIÊN ở mọi vị trí ====
            let matchedAll = true;
            for (const p of rest) {
                if (!normalizedTitle.includes(p)) {
                    matchedAll = false;
                    break;
                }
            }
            ...
            // ==== 4) Sắp xếp ưu tiên ====
            if (startsWithFirstWord) matchFirst.push(title);
            else matchAnywhere.push(title);
        }

        return [...matchFirst, ...matchAnywhere];
    }
    ...
    useEffect(() => {
        ...
        const filtered = filterSuggestions(suggestTitles, value).slice(0, 10);
